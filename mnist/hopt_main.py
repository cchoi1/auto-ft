...
class LayerLoss(nn.Module):
    ...
    def forward(self, inputs, targets, net, initial_net):
        ...
        stacked_losses = torch.stack([ce_loss, hinge_loss, entropy, l1_zero, l2_zero, l1_init, l2_init])
...
def evaluate_hparams(net, hyperparams, datasets, repeats=3, full_eval=False):
    all_val_results = []
    for _ in range(repeats):
        ...
        loss_weight_hparams = torch.tensor([hyperparams[f"lossw_{i}"] for i in range(7)])
        loss_fn = LayerLoss(loss_weight_hparams)
        train_loader = DataLoader(
            datasets["id"], batch_size=batch_size, shuffle=True, num_workers=0, drop_last=True
        )
        finetune(current_net, initial_net, optimizer, loss_fn, train_loader)
...
def hp_objective_fn(hparams):
    ...
    val_results = evaluate_hparams(_net, hparams, all_datasets)
...
default_hparams = {f"lossw_{i}": 0.0 for i in range(7)}
default_hparams["lossw_0"] = 1.0
default_hparams["lr"] = 1e-1
default_hparams["momentum"] = 0.9
print(f"\nID+OOD fine-tune baseline:")
evaluate_hparams(net, default_hparams, all_datasets_w_idood, repeats=5, full_eval=True)

print(f"\nID fine-tune baseline:")
evaluate_hparams(net, default_hparams, all_datasets, repeats=5, full_eval=True)
...
for max_evals in range(10, 100, 10):
    # logging every 10 evaluations, 100 total.
    best_hparams = fmin(
        fn=hp_objective_fn,
        space=space,
        algo=tpe.suggest,

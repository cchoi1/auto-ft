...
def get_id_dataloader(args, model, preprocess_fn):
    if args.ft_data is not None:
        ...
        return get_dataloader(dataset=id_dataset, is_train=True, args=args, image_encoder=None)


def get_val_dataset(args, preprocess_fn):
    val_dataset_class = getattr(datasets, args.id)
    val_dataset = val_dataset_class(preprocess=preprocess_fn, location=args.data_location, batch_size=args.k)
    return val_dataset


def get_ood_dataset(args, preprocess_fn):
    ood_dataset_class = getattr(datasets, args.ood)
    ood_dataset = ood_dataset_class(preprocess=preprocess_fn, n_examples=args.num_ood_hp_examples, location=args.data_location, batch_size=args.batch_size)
    return ood_dataset
...
def get_standard_datasets(args, model, preprocess_fn):
    ...
    if args.ft_data is not None:
        ...
        id_dataset = id_dataset_class(preprocess=preprocess_fn, n_examples=args.num_id_examples, location=args.data_location, batch_size=args.batch_size, num_workers=args.workers)
    id_val_dataset_class = getattr(datasets, args.id_val)
    id_val_dataset = id_val_dataset_class(preprocess=preprocess_fn, n_examples=args.num_id_examples, location=args.data_location, batch_size=args.batch_size, num_workers=args.workers)
...
def get_datasets(args, model, preprocess_fn):
    ...
    ood_subset_for_hp = get_ood_dataset(args, preprocess_fn)

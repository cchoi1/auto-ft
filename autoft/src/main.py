...
from src.models.utils import get_num_classes, test_metric_str
...
def get_fewshot_datasets(args, model, preprocess_fn):
    args.batch_size = args.k * get_num_classes(args)
...
def train(args, model, preprocess_fn):
    ...
    if args.method == "ft-id":
        ...
        model = finetune_final(args, model, loss_fn, optimizer, dataloaders, input_key, print_every=100)
...
def test(args, logger):
    ...
    for i in range(args.repeats):
        ...
        results_path = os.path.join(args.save, f"eval_results_rep{i}.json")
        with open(results_path, 'w') as f:
            f.write(json.dumps(eval_results))
        print(f'\nSaved evaluation results to {results_path}.')
        test_metrics.append(eval_results[test_metric_str(args)])
        print(f"Run {i + 1} test metric: {test_metrics}")
